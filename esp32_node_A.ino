...
const int thisNode = 0;  // Node A uses address[0]
...
void setup() {
  ...
  // Set addresses (Node A listens on address[0], sends to address[1])
  radio.openReadingPipe(0, address[thisNode]);
  radio.openWritingPipe(address[1 - thisNode]);  // Send to Node B
  radio.startListening();
