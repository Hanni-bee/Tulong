...
const byte MY_NODE_ADDR[6]   = "NODA1";
const byte PEER_NODE_ADDR[6] = "NODB1";

const char* MY_NODE_UID = "NODE_A";
const char* PEER_NODE_UID = "NODE_B";
...
void handleSyncCommand(const String& msg) {
  ...
  // App-driven RF channel selection (Channel 1=108, 2=100, 3=104, 4=112, 5=120)
  if (command == "set_rf_channel") {
    int ch = (int)(doc["rf_channel"] | 108);
    if (ch >= 0 && ch <= 125) {
      radio.setChannel((uint8_t)ch);
      if (flashStorage.begin("tulong", false)) {
        flashStorage.putUInt("rf_channel", (uint32_t)ch);
        flashStorage.end();
      }
...
void setup() {
  ...
  // RF channel: from flash (app-driven) or default
  uint32_t ch = RF_CHANNEL;
  if (flashStorage.begin("tulong", true)) {
    ch = flashStorage.getUInt("rf_channel", RF_CHANNEL);
    flashStorage.end();
  }
  if (ch > 125) ch = RF_CHANNEL;
  radio.setChannel((uint8_t)ch);
